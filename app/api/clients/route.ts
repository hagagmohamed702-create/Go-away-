...
export async function GET(request: NextRequest) {
  try {
    ...
    filteredClients.sort((a, b) => new Date(b.joinDate).getTime() - new Date(a.joinDate).getTime())
    
    // Calculate summary stats
    const totalClients = filteredClients.length
    const activeClients = filteredClients.filter(c => c.status === 'نشط').length
    const totalValue = filteredClients.reduce((sum, c) => sum + c.totalValue, 0)
    const totalContracts = filteredClients.reduce((sum, c) => sum + c.totalContracts, 0)
